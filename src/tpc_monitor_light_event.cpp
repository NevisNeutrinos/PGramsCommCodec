...
py::dict TpcMonitorLightEvent::getMetricDict() {
    ...
    metric_dict["frame_num"] = frame_num_;
    metric_dict["start_sample"] = start_sample_;
...
void TpcMonitorLightEvent::print() {
    ...
    std::cout << "  frame_num: " << frame_num_ << std::endl;
    std::cout << "  start_sample: " << start_sample_ << std::endl;
    ...
    for (size_t i = 0; i < 10 && i < tmp_vec.size(); ++i) {
        std::cout << tmp_vec[i] << (i + 1 == tmp_vec.size() ? "" : ", ");
    }
    std::cout << "..." << std::endl;
    std::cout << "++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
}
