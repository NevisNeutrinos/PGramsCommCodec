...
private:
    ...
    int32_t channel_number_;
    std::array<int32_t, DOUBLE_PACK_CHARGE_ONE_FRAME> charge_samples_{};
    // std::array<int32_t, DOUBLE_PACK_LIGHT_SAMPLE> light_samples_;
...
public:
    ...
    void setChannelNumber(int32_t channel_number) { channel_number_ = channel_number; }
    void setChargeSamples(std::array<int32_t, CHARGE_ONE_FRAME> &charge_one_frame) {
        PackDoubleWords(charge_one_frame, charge_samples_);
    }

    // --- Getter Methods ---
    const int32_t channel_number() const { return channel_number_; }
    const std::array<int32_t, DOUBLE_PACK_CHARGE_ONE_FRAME>& getChargeSamples() const { return charge_samples_; }
